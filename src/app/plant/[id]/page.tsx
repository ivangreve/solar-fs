...
export default async function PlantPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const ov = await getPlantOverview(id, user.id);
  if (!ov) {
    return (
  ...
  const series = await getPlantSeries(id, user.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatTile label="Generación hoy" to={ov.ePvKwh} kind="dec2" unit="kWh" accent="solar"
        ...
        <StatTile label="Ahorro hoy" to={ov.savings} kind="dec2" unit={ov.plant.currency} accent="money"
      ...
      <SectionCard title="Generación diaria · últimos 30 días">
